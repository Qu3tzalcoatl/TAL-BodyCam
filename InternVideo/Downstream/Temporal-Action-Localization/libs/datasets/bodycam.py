...
@register_dataset("bodycam")
class bodycam_dataset(Dataset):
    def __init__(
        self,
    ...
    def _load_json_db(self, json_file):
        ...
        for key, value in json_db.items():
            ...
            feat_file = os.path.join(self.feat_folder,
                                     self.file_prefix + key + self.file_ext)    # .lower()
        ...
        dict_db = np.array(dict_db)
        label_dict = np.array(list(label_dict.items()))
    ...
    def __getitem__(self, idx):
        ...
        print("bodycam.py L172", video_item['id'])
        

        # load features
        filename = os.path.join(self.feat_folder,
                                self.file_prefix + video_item['id'] + self.file_ext)
        print("bodycam.py (L178) Filename: ", filename)
        if os.path.exists(filename):
            feats, _, _ = read_video(filename, pts_unit='sec', output_format="TCHW").to('cuda')
            print(feats.device)
        else:
            raise Exception("File does not exist: ", filename)
        print("Original shape:", feats.shape)
        feats = feats[::self.downsample_rate, :]
        print("Downsampled shape:", feats.shape)
        # feats = torch.as_tensor(np.load(filename, mmap_mode='r').astype(np.float32), device=torch.device('cuda'))

        # feats_i3d = np.load(filename, mmap_mode='r').astype(np.float32)
        ...
        # T x C -> C x T
        feats = torch.from_numpy(np.ascontiguousarray(feats.transpose())).to('cuda')
